...
function App() {
  ...
  return (
    <div
      className="relative h-screen overflow-hidden bg-[#e8e6ed] flex items-center justify-center"
      style={{ perspective: 800 }} // enable 3D effect
    >
      <div className="relative bg-[#1c2f33] overflow-hidden h-full aspect-3/2">
        {layers.map((layer, idx) => (
          <motion.img
            key={idx}
            src={layer.src}
            alt={`Layer ${idx}`}
            className="absolute inset-0 w-full h-full top-0 left-0 object-contain"
